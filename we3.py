...
# [v56.0-ultra] - Complete Logic Synthesis & Professional Output
...
def bot_logic(query, mode="ask"):
    ...
    if mode == "ask":
        ...
        print(f"{c['orange']}◈ synthesizing complete code for: {query}...{c['reset']}")
        # ترکیب دانش از چندین منبع برای ساخت کد کامل
        full_knowledge = "\n".join([r['data'] for r in results])
        sentences = re.split(r'(?<=[.!?]) +', full_knowledge)
        tech_snippets = [s for s in sentences if any(kw in s.lower() for kw in ['config', 'kernel', 'sha', 'set', 'run', 'policy'])]
        
        script_name = f"pro_{query}.py"
        with open(script_name, "w") as f:
            f.write(f"'''\nAUTOGENERATED BY WE.PY v56.0\nTOPIC: {query}\nDATE: {time.ctime()}\n'''\n\nimport os, hashlib, sys\n\ndef security_context():\n    print('[+] implementing {query} logic based on robot brain...')\n")
            for snip in tech_snippets[:10]: # حداکثر ۱۰ جمله کلیدی را به کد تبدیل کن
                f.write(f"    # {snip[:100]}...\n")
            f.write(f"\nif __name__ == '__main__':\n    security_context()\n    print('[!] system hardened with {query} policies.')\n")
        
        print(f"{c['gold']}## professional script '{script_name}' is ready.{c['reset']}")
...
def main():
    os.system("clear")
    print(f"{c['red']}◈ we.py v56.0-ultra | the master architect ◈{c['reset']}")
    while True:
        try:
